...
function hienThiChiTietHopDong(contract) {
    ...
    if (contract.status === "<span class=\"badge bg-success\">Bản Nháp</span>") {
        actionButtons = `
            ...
            <button class="btn btn-sm btn-danger" onclick="cancelContract('${contract.id}')"><i class="fas fa-times"></i> Hủy</button>
        `;
    } else if (contract.status === "<span class=\"badge bg-info\">Đã đặt cọc</span>") {
        actionButtons = `
        ...
        `;
    } else if (contract.status === "<span class=\"badge bg-warning\">Đang Thực Hiện</span>") {
        actionButtons = `
            <button class="btn btn-sm btn-primary" onclick="payRemaining('${contract.id}')"><i class="fas fa-money-bill-wave"></i> Thanh Toán Còn Lại</button>
        `;
